...
def rel(vault_root: Path, path: Path) -> str:
...
def assess_root(vault_root: Path) -> list[Finding]:
    ...
    for child in list_children(vault_root):
...
def assess_staging(vault_root: Path) -> list[Finding]:
...
def assess_journal(vault_root: Path) -> list[Finding]:
    findings: list[Finding] = []
    root_entry_names = {child.name for child in list_children(vault_root)}
    ...
    for md in non_daily[:20]:
        findings.append(Finding("review", "JOURNAL", f"Non-daily note under JOURNAL: `{rel(vault_root, md)}`."))
    if len(non_daily) > 20:
    ...
    for md in old_year_daily[:20]:
        findings.append(Finding("review", "JOURNAL", f"Closed-year daily note not under `JOURNAL/<year>/`: `{rel(vault_root, md)}`."))
    for md in nested_current_year_daily[:20]:
        findings.append(Finding("review", "JOURNAL", f"Current-year daily note should live directly under `JOURNAL/`: `{rel(vault_root, md)}`."))
    return findings


def assess_wip(vault_root: Path) -> list[Finding]:
    findings: list[Finding] = []
    wip = vault_root / "WIP"
...
def assess_counts(vault_root: Path) -> list[Finding]:
    findings: list[Finding] = []
    for area in ["INBOX", "INBOX/LEGACY", "BACKLOG", "WIP", "MEMORY", "REPORTS", "QUARANTINE"]:
        findings.append(Finding("info", area, f"`{area}/` contains {count_md(vault_root / area)} markdown note(s)."))
    return findings


def assess_support_files(vault_root: Path) -> list[Finding]:
    findings: list[Finding] = []
    attachments = [p for p in vault_root.rglob("ATTACHMENTS") if p.is_dir()]
    canvases = [p for p in vault_root.rglob("*.canvas")]
...
def assess_sensitive_names(vault_root: Path, sensitive_re: re.Pattern[str]) -> list[Finding]:
    findings: list[Finding] = []
    matches = [p for p in vault_root.rglob("*.md") if sensitive_re.search(p.name)]
    for md in matches[:20]:
        findings.append(Finding("review", "sensitive-review", f"Filename suggests manual sensitivity review: `{rel(vault_root, md)}`."))
    if len(matches) > 20:
...
def build_report(vault_root: Path, findings: list[Finding]) -> str:
    lines = [
        "# STANDARDIZE_PROCESS",
        "",
        "## Purpose",
...
def main() -> int:
    ...
    if not vault_root.is_dir():
        ...
        return 1
    findings: list[Finding] = []
    findings.extend(assess_staging(vault_root))
    findings.extend(assess_root(vault_root))
    findings.extend(assess_journal(vault_root))
    findings.extend(assess_wip(vault_root))
    findings.extend(assess_counts(vault_root))
    findings.extend(assess_support_files(vault_root))
    sensitive_re = build_sensitive_re(args.sensitive_extra)
    findings.extend(assess_sensitive_names(vault_root, sensitive_re))
    report = build_report(vault_root, findings)
    print(report)
    if args.apply:
        output = vault_root / args.output
