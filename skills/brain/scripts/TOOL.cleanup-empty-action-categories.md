...
- Daily notes created from `_COMMON/TEMPLATES/TEMPLATE.daily-note.common.md` ship with scaffolding action categories (`* [[LEARN]]:`, `* [[READ]]:`, `* [[VIEWED]]:`, etc.). At end-of-day cleanup the unused ones should be removed while preserving categories with real content. See `BRAIN.md` § JOURNAL: *"Empty template action categories should be cleaned when a day is closed, while preserving real content and metadata"*.
