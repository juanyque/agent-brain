...
- `source_scheduler.py` — decide whether source ingestion is activated for the brain and which registered sources are due, not due, or blocked; record the watermark after a source is investigated. Dry-run by default; `mark-checked` requires `--apply` to write.
