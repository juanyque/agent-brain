...
Ask the user explicitly, per detected runtime that this hook actually supports (Claude Code and Codex only -- `session_start_hook.py` has no `--runtime` value or wiring instructions for OpenCode, Agents, or Antigravity CLI yet), whether they want the brain to connect automatically when a session starts inside it. This is a real behavior change (something executes on every session start), not a passive config link like the mappings above, so it is opt-in, never assumed -- a "no" for one runtime, or for all of them, is a legitimate answer.
