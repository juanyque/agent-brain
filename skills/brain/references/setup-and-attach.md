...
Ask the user explicitly, per detected runtime that this hook actually supports (Claude Code, Codex, and OpenCode; Agents and Antigravity CLI are not supported yet), whether they want the brain to connect automatically when a session starts inside it. This is a real behavior change (something executes on every session start), not a passive config link like the mappings above, so it is opt-in, never assumed -- a "no" for one runtime, or for all of them, is a legitimate answer.
...
- **OpenCode**: link `<agent-brain>/model/SCRIPTS/opencode_session_start_plugin.js` as `~/.config/opencode/plugins/brain-session-start.js`. Inspect that target first and never overwrite an unrelated file; create the `plugins/` directory when absent. OpenCode loads global plugins from this directory at startup. The bridge invokes `session_start_hook.py --runtime opencode` on the first `chat.message` for each session and prepends the returned context once. It deliberately does not depend on a third-party Claude-hook compatibility plugin.

Claude Code and Codex consume the same `hookSpecificOutput.additionalContext` response contract directly (verified empirically against Codex CLI 0.150.1, 2026-08-29). OpenCode uses its native plugin API and translates that response into a synthetic text part on the first message. Do not assume either contract holds for a future runtime version without checking its own documentation first.
