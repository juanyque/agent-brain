...
- `ingesta <fuente>` or `ingest <fuente>` (`$brain ingesta <fuente>` in Codex/OpenCode) → source ingestion is an optional capability (`RULES-OPTIONAL-CAPABILITIES.common.md` → "Source ingestion"); only proceed if `WIP/WIP.md`'s current-project section links `sources.registry.md`. Look up `<fuente>` there; if missing, ambiguous, or disabled, fail closed and say so. Otherwise investigate that one source now (bypassing its due-ness check), then `source_scheduler.py mark-checked --brain-root <brain> --source <fuente> --status <ok|no_activity|degraded>`.
- `revisar fuentes` or `review sources` → list the not-yet-triaged capture under `INBOX/sources/*` for the current project and let the user ask for detail on any item; this only reads and summarizes, it does not triage on its own.
