...
- Resumed or restored session (compaction restore banner, runtime relaunch with the same session id via `-s`/`--resume`/`resume <id>`, or any continuation whose session note already exists) → re-run `session_open.py` with the same session id before substantive work. It is idempotent: it reuses the existing note and registers the session in **today's** daily `# Sessions` (the real activity date, never the note's creation date). On a new calendar day the digest reports `day_rollover_detected: yes` — complete that review, then re-run with `--prepare-daily --apply`. Never create a second session note for the same session id.
