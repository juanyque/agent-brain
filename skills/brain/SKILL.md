...
- Resumed or restored session (compaction restore banner, runtime relaunch with the same session id via `-s`/`--resume`/`resume <id>`, or any continuation whose session note already exists) → re-run `session_open.py` with the same session id before substantive work. It is idempotent: it reuses the existing note and registers the session in **today's** daily `# Sessions` (the real activity date, never the note's creation date). On a new calendar day the digest reports `day_rollover_detected: yes` — complete that review, then re-run with `--prepare-daily --apply`. Never create a second session note for the same session id. If the note's Status is `consolidated` or `stale-follow-up`, apply refuses: continue in a **new** session id, or pass `--reopen-consolidated` to record an explicit reopen (`Status → open` + dated `Reopened` line) before the normal registration.
...
Do not run broad brain maintenance, standardization, or semantic reorganization from a no-argument/session-start invocation. Those require explicit arguments such as `maintain`, `clean`, `order`, `standardize`, or `init`. Exception: when today's daily note is missing, Flow 2 finalises the **previous** day (review-first TODO carry-over, then a direct-record Objectives review, then empty-category cleanup scoped to that single previous daily) and creates today's note. That per-day rollover is part of session start; the restriction here is about brain-wide maintenance, not the previous-day rollover.
