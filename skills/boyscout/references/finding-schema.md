...
| Context | The main task or investigation that surfaced this finding | Free text, e.g. `"reviewing PR #38"`, `"implementing EXAMPLE-101"`; persisted to backlog |
...
| Detected | First detection date + session context | ISO date string, e.g. `2026-04-23 · session EXAMPLE-101` — see **session label format** below |
...
| Source | Which scan mode produced this finding | `codebase` (default — `/boyscout` normal scan; omit field when this is the value) \| `deep-scan` (produced by `/boyscout deep` — interaction-context analysis of transcripts, indexed memory, and agent instructions). Useful for triage filtering (`grep "source: deep-scan"`) and to distinguish escalation patterns: deep findings often match an existing memory (`existing_memory` field), which is the signal that the memory did not fire as intended. |

**Session label format:** `YYYY-MM-DD · session <ticket-or-tool-label>` where `<ticket-or-tool-label>` is one of:

- A ticket code when the scan happened during ticket work: `session EXAMPLE-101`
- The literal `boyscout` for standalone scans not tied to a ticket: `session boyscout`
- Either of the above with a parenthetical when context helps: `session boyscout (EXAMPLE-101 wrap-up)`, `session EXAMPLE-102 PR review`
...
(e.g. `example-plugin/pr-review/SKILL.md`) or for user skills:
...
The `<plugin-name>` is the directory prefix under the skills folder (e.g. `example-plugin`,
`user-skill`); it corresponds to the namespace before the colon in the skill's display name
(e.g. `example-plugin` in `example-plugin:pr-review`).
Use the canonical skill name from its frontmatter, not a machine-local directory alias.
...
- When in doubt, file-level granularity (`AGENTS.md`) is acceptable — a PR touching one file is still a valid grouping
...
- `agent-skills / <plugin>/<skill>` — when the action updates a skill (e.g. `agent-skills / example-plugin/pr-review`).
- `agent-config / <instruction-file>` — when the action updates a runtime or project instruction file (clarify which one in `location`).
...
In `/boyscout deep` mode the cap is **shared across the three detector passes** — not 10 per type. Each
detector also independently caps at 10 (declared in its `detection-*.md` "Output cap" section),
...
```
- occurrences: 2026-05-18 (EXAMPLE-101); 2026-05-19 (EXAMPLE-102)
```
