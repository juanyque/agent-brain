...
4. The consolidation checklist is complete, or any unchecked item has an explicit written reason in the new session note or `JOBS_LOGS.md`. Do not trust the checklist's own checkboxes as proof: `session_close.py consolidate` verifies the "Session ID written in daily note" item against the real `JOURNAL/*.md` files on disk and prints `verified` or `WARNING` accordingly — read that line rather than assuming the box was checked correctly. The same command also re-checks `WIP/SOURCES/` against `source_scheduler.py`'s own due-ness logic and warns if any enabled, unblocked source is still due and was never investigated this session — the same "opening a session lists what's due, but nothing forces it to actually happen" gap the source-ingestion capability itself doesn't close on its own.
