...
**Delegate the read-classify-draft pass; keep every write serial in the calling agent.** Reading a peer session note, deciding whether it's finished, and drafting where its durable content belongs is informative work about the past — a good fit for parallel subagents, the same "reference knowledge, not behavior rule" case used elsewhere in this model. Actually writing to `WIP/WIP.md` or a daily note is different: those are shared files another session may also touch, and this vault has already lost a whole `[[TODO]]` section once to two sessions writing the same file at once (see "Paralelizar lectura es gratis..." above) — so writes never run in parallel, regardless of how the reads were parallelized.
